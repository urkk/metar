...
// PossiblePhemonenaCode - Possible combinations of codes and modifiers. Not all codes can be combined. So the VC can only be applied to SH, TS, FG, VA, BLDU, BLSA, BLSN, PO, FC, SS, DS. It's impossible to have a "light" tornado (-FC), ground only fog (MIFG), etc. See documentation.
...
// PossibleRecentPhemonenaCode - Not all phenomena are acceptable as recent. No indication of intensity.
...
// UniqueCodes - A list of unique codes to convert groups such as TSRASNGR to Thunderstorm with rain (TSRA), snow (SN), hail (GR)
...
// Intensity - intensity of weather phenomena
...
type Phenomena []Phenomenon

// AppendPhenomena - Check whether the string is a description of phenomena. If successful, adds a new phenomenon
func (pha *Phenomena) AppendPhenomena(input string) bool {
	if p := ParsePhenomena(input); p != nil {
		*pha = append(*pha, *p)
		return true
	}
	return false
...
// AppendRecentPhenomena - Check whether the string is a description of recent phenomena. If successful, adds a new phenomenon
func (pha *Phenomena) AppendRecentPhenomena(input string) bool {
	if p := ParseRecentPhenomena(input); p != nil {
		*pha = append(*pha, *p)
		return true
	}
	return false
...
// ParsePhenomena - returns a set of encoded weather events with modifiers or a nil pointer
func ParsePhenomena(token string) (p *Phenomenon) {
...
// ParseRecentPhenomena - Recent phenomena can't be in the vicinity and have qualifier
func ParseRecentPhenomena(token string) (p *Phenomenon) {
